...
namespace MakakaGames
{
    ...
    {
        private static readonly string dialogTitle = "GUIDs regeneration";
        private static readonly string dialogMessage =
        ...
        private static readonly string dialogOk = "Regenerate GUIDs";
        private static readonly string dialogCancel = "Cancel";

        [MenuItem("Window/Makaka Games/" +
            "Regenerate GUIDs for assets in specific folder", false, 100)]
        public static void RegenerateGUIDsForAssetsInSpecificFolder() 
        {
            string pathFolder = EditorUtility.OpenFolderPanel(
                "Choose folder for regenerating assets GUIDs",
                "Assets/",
                string.Empty);

            if (!string.IsNullOrEmpty(pathFolder)
                && EditorUtility.DisplayDialog(
                    dialogTitle, dialogMessage, dialogOk, dialogCancel)) 
            {
                try 
                {
                    AssetDatabase.StartAssetEditing();

                    GUIDRegenerator regenerator = new(pathFolder);
    ...
    internal class GUIDRegenerator
    {
        ...
        public void RegenerateGUIDs(string[] regeneratedExtensions = null) 
        {
            ...
            foreach (string extension in regeneratedExtensions) 
            {
                filesPaths.AddRange(
                    Directory.GetFiles(
                        _assetsPath, extension, SearchOption.AllDirectories));
            }
            ...
            // Otherwise built-in resources (shader, meshes,..) get overwritten.
            HashSet<string> ownGuids = new();

            // Traverse all files, remember which GUIDs are in which files
            // and generate new GUIDs
            int counter = 0;
            ...
            foreach (string filePath in filesPaths) 
            {
                    ...
                    counter / (float) filesPaths.Count)) 
                {
                    ...
                    {
                        // First GUID in .meta file is always the GUID
                        // of the asset itself
                        if (isFirstGuid
                        ...
                        // Generate and save new GUID
                        // if we haven't added it before
        ...
        private static IEnumerable<string> GetGUIDs(string text) 
        {
            ...
            {
                index = text.IndexOf(
                    guidStart, index, StringComparison.Ordinal);
